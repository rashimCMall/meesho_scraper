...
async function getData() {
  try {
    for(const category in categories) {
      ...
      for(const d of data) {
        const query = `insert into ${category} (image,name,price,discount,rating) values ($1, $2, $3, $4, $5)`;

        pool.query(query, d, (error) => 
        {
          if (error) {
            throw error
          }
